...
// Long-running Railway process: without this, `cooldowns` grows by one
// entry per unique (scope, userId) pair forever and never shrinks, since
// checkCooldown only ever adds/overwrites keys. Sweep expired entries on
// an interval instead of on every check, to keep the hot path cheap.
let sweepTimer = null;
function startCooldownSweep(intervalMs = 5 * 60 * 1000) {
...
module.exports = { checkCooldown, clearCooldown, formatRemaining, startCooldownSweep, stopCooldownSweep };
